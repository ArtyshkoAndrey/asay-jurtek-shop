...
@section('title', 'Редактирование ссылки для категории ' . $linkMenu->category->name)
  ...
  <div class="container-fluid pt-5 px-4">
    ...
    <div class="row mt-0 pt-0">
      <div class="card border-0 w-100 rounded-0" style="z-index: 90;box-shadow: 0 18px 19px rgba(0, 0, 0, 0.25)">
        ...
        <div class="card-body">
          <form action="{{ route('admin.setting.link-menu.update', $linkMenu->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="row mt-3 align-items-end">
              <div class="col-md-3">
                ...
                <input type="text" name="name" id="name" class="w-100 px-2 form-control rounded-0 {{ $errors->has('name') ? ' is-invalid' : '' }}" value="{{ old('name', $linkMenu->name) }}" required>
              ...
              <div class="col-md-3">
                ...
                <select name="category_id" id="category" class="w-100 px-2 form-control rounded-0 {{ $errors->has('category_id') ? ' is-invalid' : '' }}">
                  @foreach($categories as $cat)
                    <option value="{{ $cat->id }}" {{ old('category_id', $linkMenu->category->id) === $cat->id ? 'selected' : null }}>{{ $cat->name }}</option>
                  @endforeach
                </select>
