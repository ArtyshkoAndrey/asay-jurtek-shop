      ...
      <div class="card border-0 w-100 rounded-0" style="z-index: 90;box-shadow: 0 18px 19px rgba(0, 0, 0, 0.25)">
        ...
        <div class="card-body">
          <form action="{{ route('admin.news.update', $news->id) }}" method="post">
            @csrf
            @method('PUT')
            <input type="hidden" name="image" value="{{ old('image', $news->image) }}">
            ...
            <div class="row">
              <div class="col-md-6">
                ...
                <input type="text" class="form-control rounded-0 {{ $errors->has('title') ? ' is-invalid' : '' }}" name="title" id="title" value="{{ old('title', $news->title) }}" required>
                <span id="title-error" class="error invalid-feedback">{{ $errors->first('title') }}</span>
              </div>

              <div class="col-md-6">
                <label for="description">Краткое описание</label>
                <textarea name="description" id="description" class="form-control rounded-0 {{ $errors->has('description') ? ' is-invalid' : '' }}" cols="10" rows="2">{{ old('description', $news->description) }}</textarea>
                <span id="description-error" class="error invalid-feedback">{{ $errors->first('description') }}</span>
              </div>

              <div class="col-md-12">
                <label for="content">Контент</label>
                <textarea name="content" id="content" class="form-control rounded-0 {{ $errors->has('content') ? ' is-invalid' : '' }}">{{ old('content', $news->content) }}</textarea>
...
@section('js')
  ...
  <script>
    ...
    @if(old('image', $news->image))
      fileList = {"serverFileName": '{{ old('image', $news->image) }}', "fileName": '{{ old('image', $news->image) }}'};
      let mockFile = { name: '{{ old('image', $news->image) }}', size: 0 };
      uploader.emit("addedfile", mockFile);
      uploader.emit("thumbnail", mockFile, '{{ asset('storage/news') . '/' . old('image', $news->image) }}');
